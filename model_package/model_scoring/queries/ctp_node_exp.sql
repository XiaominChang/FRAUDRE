 ...
 /* ============================================
    1) Claim / Exposure + Contact snapshot
    ============================================ */
DROP TABLE IF EXISTS cte_claim_info;
CREATE TEMP TABLE cte_claim_info AS
(
    SELECT DISTINCT 
    	a.claim_number,
    	b.claim_exposure_id,
 		MAX(b.claim_exposure_lodgement_date) AS claim_exposure_lodgement_date,
        MAX(b.claim_loss_date) AS claim_exposure_loss_date,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    2) Injury info (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS cte_injury_info;
CREATE TEMP TABLE cte_injury_info AS
(
    SELECT
        mccs.claim_number,
        mccs.claim_exposure_id::INT AS claim_exposure_id,
        MAX(CASE WHEN mccs.was_not_fit_for_work_flag = 'Y' THEN 1 ELSE 0 END) AS not_fit_for_work_flag,
        MAX(CASE WHEN mccs.estimate_of_injury_severity_name ILIKE '%Minor%'
                       AND mccs.was_not_fit_for_work_flag = 'Y' THEN 1 ELSE 0 END) AS off_work_minor_injury_flag
    FROM pub.mv_ctp_claim_summary mccs
    WHERE mccs.claim_exposure_id IS NOT NULL
      AND mccs.statutory_state = 'NSW'
    GROUP BY mccs.claim_number, mccs.claim_exposure_id
)
DISTRIBUTED BY (claim_number);

 /* ============================================
    3) Medical info (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS cte_medical_info;
CREATE TEMP TABLE cte_medical_info AS
(
    SELECT
        a.claim_number,
        a.claim_exposure_id::INT AS claim_exposure_id,
        CASE WHEN a.minor_injury_decision_code = '0' THEN 1 ELSE 0 END AS not_threshold_injury_flag,
        CASE
            WHEN a.minor_injury_assessment_type_name IS NOT NULL
            THEN (CAST(a.date_of_minor_injury_assessment_decision AS DATE) - CAST(b.claim_lodgement_date AS DATE))
            ELSE 0
        END AS days_of_minor_assess_to_lodgement
    FROM pub_core.claim_injury a
    INNER JOIN pub_core.mv_claim_header b
            ON a.claim_number = b.claim_number
    WHERE b.policy_issue_state = 'NSW'
      AND b.line_of_business_name = 'Compulsory Third Party'
)
DISTRIBUTED BY (claim_number);

 /* ============================================
    4) Injured person employment (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS injured_employment_info;
CREATE TEMP TABLE injured_employment_info AS
(
    SELECT 	claim_number,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    5) Fitness certificate (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS cte_fitness_certificate;
CREATE TEMP TABLE cte_fitness_certificate AS
(
	SELECT 
	    a.claim_number,
	    CAST(claim_exposure_id AS INT) AS claim_exposure_id,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    6) COF issue/received timing (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS cof_issue_info;
CREATE TEMP TABLE cof_issue_info AS
(
        SELECT 
            a.claim_number,
            CAST(claim_exposure_id AS INT) AS claim_exposure_id,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    7) Rejected payment > 8 (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS reject_payment_invoice;
CREATE TEMP TABLE reject_payment_invoice AS
(
    SELECT
        a.claim_number,
        a.claim_exposure_id::INT AS claim_exposure_id,
        a.invoice_number,
        CASE WHEN SUM(CASE WHEN a.transaction_status_code = 'Rejected' THEN 1 ELSE 0 END) > 8 THEN 1 ELSE 0 END AS rejected_pay_over_8_flag
    ...
    GROUP BY a.claim_number, a.claim_exposure_id, a.invoice_number
)
DISTRIBUTED BY (claim_number);

DROP TABLE IF EXISTS reject_payment_info;
CREATE TEMP TABLE reject_payment_info AS
(
    SELECT
        claim_number,
        claim_exposure_id,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    8) GP payments (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS gp_payment_expo;
CREATE TEMP TABLE gp_payment_expo AS
(
    SELECT
        claim_number,
        claim_exposure_id,
        SUM(CASE WHEN transaction_subtype_name ILIKE '%General Practitioner Consultation%' THEN account_amount ELSE 0 END) AS gp_consult_payment_total,
        SUM(CASE WHEN transaction_subtype_code = 'GP05' THEN account_amount ELSE 0 END) AS non_pre_approved_gp_case_total
...
DROP TABLE IF EXISTS gp_payment_info;
CREATE TEMP TABLE gp_payment_info AS
(
    SELECT
        a.claim_number,
        a.claim_exposure_id::INT AS claim_exposure_id,
        CASE WHEN b.non_pre_approved_gp_case_total > b.gp_consult_payment_total THEN 1 ELSE 0 END AS gp05_pay_exceed_gp_consult
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    9) Investigation flags (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS investigation_info;
CREATE TEMP TABLE investigation_info AS
(
    SELECT claim_number,
        CAST(claim_exposure_id AS INT) AS claim_exposure_id,
...
DISTRIBUTED BY (claim_number);
 ...
 /* ============================================
    10) FINAL FEATURES (exposure-level)
    ============================================ */
DROP TABLE IF EXISTS ctp_exposure_features_dbt;
CREATE TEMP TABLE ctp_exposure_features_dbt AS
(
    SELECT DISTINCT 
        a.*,
        ...
        CAST(d.max_pre_accident_weekly_earning AS FLOAT) AS pre_accident_weekly_earning,
...
SELECT * FROM ctp_exposure_features_dbt 
